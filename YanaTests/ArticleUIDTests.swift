...
struct ArticleUIDTests {
    ...
    func collapseCountsUTF16Units() {
        // The bound is measured in UTF-16 code units, so 130 emoji (130 characters,
    ...
    func shortUIDUnchanged() {
        // Existing rows must keep their identity, so anything within the limit is returned
        // untouched rather than hashed.
        let uid = ArticleUID.make(
