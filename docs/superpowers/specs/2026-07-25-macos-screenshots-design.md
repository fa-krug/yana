...
- `MacRootView` — `mac.window.root`, `mac.sidebar.list`. The sidebar search field gets **no**
  identifier: `.searchable` does not forward one reliably, so the test matches it by element type
  as `app.searchFields`, which is already locale-independent.
...
| `02_Search` | Type "battery" into `app.searchFields`; assert a row matching the term appears | `02_Search.png` |
